...
describe("grok.terminal", function()
  ...
  describe("build_cmd", function()
    it("allows edit tools by default (the Neovim diff gate reviews them)", function()
      config.setup({})
      assert.are.same({ "grok", "--allow", "Edit", "--allow", "Write" }, terminal.build_cmd())
    end)
    ...
    it("permission_mode auto wins over the edit allow rules", function()
